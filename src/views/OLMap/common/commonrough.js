...
/**
 * @module commonrough
 * @description 已抽希的地图图层相关操作函数
 * */

/**
 * 通过地图行政区划编码获取系统行政区划编码
 * @param {String} adcd - 地图行政区划编码
 * @returns {string} 系统行政区划编码
 */
...
/**
 * 通过行政区划等级获取地图图层
 * @param {Number} adLevel - 行政区划等级
 * @returns {string} 地图图层名称
 */
export function getFeatureTypesByAdLevel(adLevel) {
  let featureTypes = []
  switch (adLevel) {
    case 1:
      featureTypes = 'ZhejiangAdminDivisionRough:province'
      break
    case 2:
      featureTypes = 'ZhejiangAdminDivisionRough:city'
      break
    case 3:
      featureTypes = 'ZhejiangAdminDivisionRough:county'
      break
    default:
      featureTypes = 'ZhejiangAdminDivisionRough:town'
  }
  return featureTypes
}

/**
 * 通过行政区划等级获取下级行政区划地图图层
 * @param {Number} adLevel - 行政区划等级
 * @returns {string} 地图图层名称
 */
