...
import { shadeLayer_coord } from '../config/layerConfig'
import {
...
class ShadeLayer {
  ...
  async load(adcd, map, needfit) {
    // 加载边界数据
    if (!this.layer) {
      this.layer = new BaseVectorLayer(shadeLayer_coord)
    ...
    const result = await getGisDataByWfs(params, `${geoServerConfig.url}/ZhejiangAdminDivision/wms`)
