...
import { waterAreaLayer } from '../config/layerConfig'
import * as common from '../common/common'
import { geoServerConfig } from '../config/mapConfig'
import GeoJSON from 'ol/format/GeoJSON'
import { getCenter } from 'ol/extent'

class WaterAreaLayer extends BaseVectorLayer {
  constructor() {
    super(waterAreaLayer)
  }
  async load(params) {
    ...
    let res = await getWaterAreaInfo(querys)
    res = res.message
    ...
    // 把数据进行处理
    if (res.length > 0) {
      vm.waterAreass = res.filter(element => {
        return element.totalChangeArea > 0 || element.totalChangeArea < 0
      })
      // 获取变化的最大最小值
      vm.waterAreass.forEach(water => {
        if(Math.abs(water.totalChangeArea) > maxNum && Math.abs(water.totalChangeArea) > 0) {
          maxNum = Math.abs(water.totalChangeArea)
        }
        if(Math.abs(water.totalChangeArea) < minNum && Math.abs(water.totalChangeArea) > 0) {
          minNum = Math.abs(water.totalChangeArea)
        }
      })
      vm.waterAreass.forEach(water => {
        water.maxNum = maxNum
      ...
      let fts = new GeoJSON().readFeatures(result)
      fts.forEach(ft => {
        vm.waterAreass.forEach(waterArea => {
          if (waterArea.adcd === ft.get('admin_div_code')) {
            waterArea.lgtd = getCenter(ft.getGeometry().getExtent())[0]
            waterArea.lttd = getCenter(ft.getGeometry().getExtent())[1]
          }
        })
      })
      vm.$nextTick(() => {
        vm.waterAreas = vm.waterAreass
      })
      console.log('waterAreass', vm.waterAreass);
    }
  }
