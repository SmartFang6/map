...
import { warningDikeCircleLayer } from '@/views/OLMap/config/layerConfig'

class WarnDikeAnimatePointLayer extends MergeLayerImpl {
  constructor(layerConfig) {
    super(layerConfig)
    this.echartsLayer = null
    this.warningAreaCircle = new NearByCircleLayer(warningDikeCircleLayer)// 添加圆样式
  }
  ...
  async load(params) {
    ...
    this.legendChange(null, vm)
// 添加圆样式
    this.warningAreaCircle.load(map, this.res, 5000)
    map.getView().fit(this.warningAreaCircle.getSource().getExtent()) // 视图定位
  }
  ...
  //     datas = this.res.filter(element => {
  //       element.color = 'green'
  //           // properties.desc = properties.totalRainValue + 'mm'
  //           element.config = [{
  //             label: '近24小时',
  //             key: 'rain',
  //             unit: 'mm'
...
export default WarnDikeAnimatePointLayer
